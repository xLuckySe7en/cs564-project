...
private:
  ...
  int hash(const File *file,
           const int pageNo); // returns value between 0 and HTSIZE-1
...
public:
  ...
  const Status
  flushFile(const File *file); // writing out all dirty pages of the file
  ...
  const void clearBufStats() { bufStats.clear(); }
};
