...
#define EXIT                                                                   \
  do {                                                                         \
    fprintf(stderr, "!!! ERROR EXIT ON %s:%d", __FILE__, __LINE__);            \
    exit(1);                                                                   \
  } while (0)
...
int main() {
  ...
  cout << "Start an insert scan which will have the side effect of opening "
          "dummy.02 "
       << endl;
  ...
  cout << "inserted " << num << " records into file dummy.02 successfully"
       << endl;
  cout << endl;

  cout << endl;
  cout << "pull 11th record from file dummy.02 using file->getRecord() "
       << endl;
  ...
  cout << endl;
  cout << "pull every 7th record from file dummy.02 using file->getRecord() "
       << endl;
  file1 = new HeapFile("dummy.02", status); // open the file
  if (status != OK) {
    error.print(status);
  ...
  for (i = 0; i < num; i++) {
    ...
    if (i == 0 || rec1Len < smallest)
      smallest = rec1Len;
    if (i == 0 || rec1Len > largest)
      largest = rec1Len;
  }
  cout << "inserted " << num
       << " variable sized records successfully into dummy.03" << endl;
  ...
  if (status != OK) {
    ...
    while ((status = scan1->scanNext(rec2Rid)) != FILEEOF) {
      ...
      if (rec2.i >= j || rec2.f != dbrec2.length ||
          rec2.s[0] != 32 + dbrec2.length - 8) {
        cout << "err0r reading record " << i << " back" << endl;
        EXIT;
      }
      i++;
    ...
    if (i != num / 2) {
      cout << "Err0r.   scan should have returned " << num / 2 << " records!"
           << endl;
      EXIT;
    }
  }
  ...
  status =
      scan2->startScan(Foffset, sizeof(float), FLOAT, (char *)&Fvalue, GTE);
  if (status != OK) {
    error.print(status);
    EXIT;
  } else {
  ...
  int numDeletes = 0;
  cout << endl;
  cout << "now scan dummy.04, deleting records with keys between 1000 and 2000"
       << endl;
  ...
  if (status != OK) {
    ...
    cout << "scan file1 saw " << i << " records " << endl;
    if (i != num) {
      cout << "Err0r. scan should have returned " << (int)num << " records!!"
           << endl;
      EXIT;
    }
  ...
  cout << endl
       << "Filtered scan matching i field GTE than " << filterVal1 << endl;
  ...
  if (status != OK) {
    ...
    while ((status = scan1->scanNext(rec2Rid)) != FILEEOF) {
      ...
      if (!(currRec->i >= filterVal1)) {
        cerr << "Err0r.   filtered scan returned record that doesn't satisfy "
                "predicate "
  ...
  cout << endl
       << "Filtered scan matching f field GT than " << filterVal2 << endl;
  status = scan1->startScan(sizeof(int), sizeof(float), FLOAT,
                            (char *)&filterVal2, GT);
  if (status != OK) {
    cerr << "got err0r status return from startScan" << endl;
    error.print(status);
    ...
    cout << "scan file1 saw " << i << " records " << endl;
    if (i != num / 10 - 1) {
      cout << "Err0r.   filtered scan 2 should have returned " << num / 10 - 1
           << " records!" << endl;
      EXIT;
    }
  }
